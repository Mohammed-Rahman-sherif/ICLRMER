...
import config_iemocap as config
...
#         return logits
    ...
    def forward(self, x_dict, edge_index_dict, speaker_idx_tensor, batch_dict):
        ...
        # per-modality projection + per-dialogue Transformer
        for node_type, x_features in x_dict.items():
            x_with_speakers = torch.cat([x_features, speaker_embs], dim=-1)
            projected_features = self.input_proj_dict[node_type](x_with_speakers)
            ...
            batch = batch_dict[node_type]
            dialogue_outputs = []
            for dialogue_idx in torch.unique(batch):  # or torch.unique_consecutive(batch)
                mask = (batch == dialogue_idx)
                seq = projected_features[mask].unsqueeze(0)  # [1, T, C]
                tr_out = self.transformer_encoders[node_type](seq)
                dialogue_outputs.append(tr_out.squeeze(0))   # [T, C]
        ...
        for layer_idx, (conv, norm_dict) in enumerate(zip(self.convs, self.norms)):
            ...
            current_x_dict = {
                nt: self.dropout(norm_dict[nt](F.relu(x)))
                for nt, x in current_x_dict.items()
            }
